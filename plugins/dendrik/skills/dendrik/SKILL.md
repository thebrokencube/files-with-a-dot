...
dendrik is the shared foundation the dotfiles CLI tools (folio, jf, dot) are built on. This skill is dendrik's review + lint surface; its focused references below are self-contained in the plugin bundle.
...
| `/dendrik review <file\|repo\|PR>` or any reviewable doc (incl. SKILL.md) | **Review** — skill workflow; detect type → dispatch |
...
| `references/blueprint.md` | dendrik's own blueprint — which dendrik concepts it composes (owner/dogfood; each *other* system's blueprint lives in that system's own plugin) |
