...
The publishable plugin bundle contains only the native manifest, canonical skill tree, `bin/setup`, and `VERSION`; plugin delivery content does not remain under `cmd/<tool>`.

**Fix**: Move delivery content to `plugins/<tool>` and remove source, tests, build files, runtime state, or private material from the bundle.
