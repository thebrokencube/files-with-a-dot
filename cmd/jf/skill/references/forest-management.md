...
- **Sync conflict pre-scan**: checks `sync:both` nodes before push/pull. Skips conflicts unless `--resolve` is set.
- **`--force` on create-missing**: pushes plain text if marklassian fails, does NOT skip dedup check
